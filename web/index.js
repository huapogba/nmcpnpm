...
app.put('/api/nhanvien/:sdt', async (req, res) => {
  const sdtParam = req.params.sdt;
  ...
  try {
    ...
    const resultCheck = await requestSelect
      .input('sdtParam', sql.VarChar(10), sdtParam)
      .query('SELECT cccd FROM dbo.NhanVien WHERE sdt = @sdtParam');
    ...
    const oldCCCD = resultCheck.recordset[0].cccd;
    ...
    res.status(200).json({
      ...
      nhanvien: { ho_ten, ngay_sinh, dia_chi, sdt, email, chuc_vu, trang_thai }
    });
