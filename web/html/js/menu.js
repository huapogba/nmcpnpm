...
function openAddForm(category) {
  ...
  // Reset các input cơ bản
  document.getElementById('dish-name').value = '';
  document.getElementById('dish-price').value = '';
  document.getElementById('dish-img').value = '';

  // Reset phần nguyên liệu (nếu có input nguyên liệu động)
  const ingredientsContainer = document.getElementById('ingredients-container');
  // Nếu bạn thêm input nguyên liệu bằng JavaScript, xóa hết input con ở đây
  ingredientsContainer.querySelectorAll('.ingredient-input').forEach(el => el.remove());

  // Gán lại giá trị category
...
function openEditForm(category, idx) {
  ...
  if (!dish) {
    showModal('Món ăn không tồn tại!');
    return;
  }
...
async function submitDish() {
  ...
  if (!name || isNaN(price) || price <= 0 || isNaN(category)) {
    showModal('Vui lòng nhập đầy đủ và hợp lệ tên, giá, thể loại món ăn.');
    return;
  }
  ...
  try {
    ...
    showModal(data.message || (isEditing ? 'Cập nhật món ăn thành công!' : 'Lưu món ăn thành công!'));
    ...
    console.error(error);
    showModal(error.message || 'Lỗi khi lưu món ăn');
  }
}
...
async function saveOrderToHistory(cartItems) {
  const orderId = 'DH' + Date.now(); // mã đơn hàng
  const orderDate = new Date().toISOString(); // ISO string cho server
  const total = cartItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const payment = 'Chưa thanh toán'; // hoặc lấy từ form nếu có
  ...
  // Lấy token từ localStorage hoặc nơi lưu token khi login
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('btn-delete-yes').addEventListener('click', async () => {
    ...
    try {
      await xoaMon(dish.id); 
      menuData[category].splice(index, 1);
      renderCategory(category);
      showModal('Xóa món thành công!');
    } catch (error) {
      showModal('Xóa món không thành công: ' + error.message);
    }
...
function setupUIByRole(chucVu) {
  ...
  const roleButtonsMap = {
    ...
    'Nhân Viên bếp' :['btn-menu','btn-lichsu','btn-xemykien','btn-lichlamviec','btn-logout'],
